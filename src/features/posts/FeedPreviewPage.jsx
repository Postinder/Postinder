...
export default function FeedPreviewPage() {
  ...
  const filtered = filter ? posts.filter(p => p.client_id === filter) : posts
        ...
        <>
          <div className="grid grid-cols-3 gap-1 max-w-sm mb-4">
            {filtered.map(p => {
              ...
              const url = p.files?.[0]?.storage_url
              return (
          ...
          <div className="flex gap-4 flex-wrap text-xs text-neutral-400">
            {[['bg-green-500','Aprovado'],['bg-red-500','Recusado'],['bg-neutral-400','Pendente'],['bg-blue-500','Atualizado']].map(([c,l]) => (
