  ...
  return (
    <div className="grid grid-cols-2 overflow-hidden rounded-xl border border-neutral-200 bg-white shadow-sm dark:border-neutral-800 dark:bg-neutral-900 xl:grid-cols-4">
      {items.map(({ id, icon, label, value, sub }) => (
        ...
        >
          <div className="flex items-center gap-2 text-neutral-400 dark:text-neutral-300/80">
            <span className="text-[var(--portal-brand-foreground)]" aria-hidden="true">{icon}</span>
          ...
          {sub ? <div className="mt-0.5 truncate text-[11px] text-neutral-500 dark:text-neutral-300/80">{sub}</div> : null}
        </div>
      ))}
    </div>
  )
