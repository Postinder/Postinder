...
export default function NewPostPage() {
  ...
  const [linkModal, setLinkModal] = useState({ open:false, link:'', clientPhone:'', clientName:'' })
  ...
  async function handleSave() {
    ...
    try {
      ...
      setLinkModal({ open:true, link, clientPhone, clientName })
    ...
    <div>
      ...
      <Modal open={linkModal.open} onClose={()=>{ setLinkModal({open:false,link:''}); navigate('/admin/dashboard') }} title="✅ Postagem criada!" subtitle="Copie o link e envie para o cliente pelo canal de sua preferência.">
        <div className="bg-neutral-50 dark:bg-neutral-800 rounded-xl p-4 mb-4">
          ...
          <code className="block text-xs text-blue-600 dark:text-blue-400 break-all font-mono leading-relaxed mb-3">{linkModal.link}</code>
          {linkModal.clientPhone ? (
            <button
              onClick={() => {
                let phone = linkModal.clientPhone.replace(/\D/g,'')
                if (!phone || phone.length < 10) { toast.error('Número de WhatsApp inválido.'); return }
                if (phone.length <= 11) phone = '55' + phone
                const msg = encodeURIComponent(`Olá ${linkModal.clientName}! 📋 Você tem conteúdos aguardando aprovação. Acesse o link abaixo:\n\n${linkModal.link}`)
                window.open(`https://wa.me/${phone}?text=${msg}`, '_blank')
              }}
              className="w-full flex items-center justify-center gap-2 bg-[#25D366] hover:bg-[#20ba5a] text-white font-bold py-2.5 rounded-xl text-sm transition-colors">
              <MessageCircle size={16}/> Enviar pelo WhatsApp
            </button>
          ) : (
            <button onClick={copyLink} className={`w-full flex items-center justify-center gap-1.5 px-3 py-2.5 rounded-xl text-sm font-bold text-white transition-all ${copied?'bg-green-500':'bg-teal-500 hover:bg-teal-600'}`}>
        ...
        {!linkModal.clientPhone && (
          <div className="bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800 rounded-lg p-3 text-xs text-amber-700 dark:text-amber-400 mb-4">
            💡 Cadastre o WhatsApp do cliente para envio direto.
          </div>
        )}
        <div className="flex gap-3">
