...
- A migration `016_client_documents.sql` foi aplicada com sucesso. O banco publicado esta em `016`; as migrations `017` a `023` existem somente no codigo local e devem ser aplicadas, nessa ordem, antes de um futuro deploy autorizado.
...
- O pacote local de aprovacao configuravel, revisao/certificacao, snapshot do funil e historico append-only de soundtrack foi auditado e validado, mas ainda nao foi commitado nem publicado. O proximo deploy deve aplicar `017` a `023` antes de iniciar o backend novo.
...
Para o rollout do pacote local: (1) criar backup e executar preflight; (2) aplicar, em ordem, as migrations aditivas `017` a `023`; (3) publicar o backend; (4) concluir health checks e smoke tests; (5) publicar o frontend. Em rollback de aplicacao, retornar primeiro o frontend e depois o backend; nao remova migrations ja aplicadas nem edite `schema_migrations` manualmente.
