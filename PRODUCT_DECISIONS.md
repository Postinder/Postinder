...
- A acao negativa apresentada ao Cliente e **Solicitar ajuste**. Os motivos atuais sao Design, Foto, Video, Legenda, Texto do conteudo, Titulo/chamada e Outro; no modo `item`, comentario obrigatorio e tags ficam associados a midia ajustada.
...
- **Adorei** pertence exclusivamente ao fluxo de aprovacao do conteudo. Soundtrack continua oferecendo apenas aprovacao ou solicitacao de ajuste e nao recebe `positive_reaction`.
...
- A metrica **Adorei no mes** conta uma vez por post atualmente certificado quando a projecao oficial corrente possui `loved` no conteudo ou em ao menos um item e `approvedAt` pertence ao mes de referencia. Nao conta itens individualmente, retries, aprovacao normal, rejeicao, legado sem `loved`, `loved` fora do mes ou decisoes antigas que deixaram de ser a projecao corrente depois de rewind. Varios itens adorados no mesmo post contam somente uma vez.
